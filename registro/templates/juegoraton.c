...
// Encuentra la posición actual del ratón
for (int i = 0; i < FILAS; i++) {
    for (int j = 0; j < COLUMNAS; j++) {
        if (tablero[i][j] == 'R') {
            filaRaton = i;
...
// Pide al usuario la nueva posición
int nuevaFilaRaton, nuevaColumnaRaton;
printf("Ingrese la nueva fila y columna del ratón: ");
scanf("%d %d", &nuevaFilaRaton, &nuevaColumnaRaton);
// Actualiza el tablero
tablero[filaRaton][columnaRaton] = '.';
tablero[nuevaFilaRaton][nuevaColumnaRaton] = 'R';
